Select j.job_id, j.Location_id, Location_name, t.time_year, t.time_month,
	Sum(cost_labor) As Sum_labor_cost, Sum(cost_material) As Sum_material_cost, Sum(cost_overhead) As Sum_overhead_cost,
	Sum(machine_hours)* Sum(rate_per_hour) As Sum_machine_cost, 
	Sum(cost_labor)+Sum(cost_material)+Sum(cost_overhead)+(Sum(machine_hours)*Sum(rate_per_hour)) As Sum_total_cost, 
	Sum(quantity_produced) As Sum_qty_produced, 
	(Sum(cost_labor)+Sum(cost_material)+Sum(cost_overhead)+(Sum(machine_hours)*Sum(rate_per_hour)))/Sum(quantity_produced) As Unit_cost
