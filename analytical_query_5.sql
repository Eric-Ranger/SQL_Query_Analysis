Select irs.job_id, irs.location_name, irs.time_year,
	Sum(sum_invoice_amt - sum_total_cost)/Sum(sum_invoice_amt) As profit_margin,
	Percent_RANK()OVER (
		ORDER BY (Sum(sum_invoice_amt - sum_total_cost)/Sum(sum_invoice_amt))DESC) AS profit_Rank
